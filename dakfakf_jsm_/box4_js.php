	...
	<script type="text/javascript">
					...
					if (bawah != 1) {
						...
						if(data[0]['akses']=='Y'){
							if ($('#tablbox4').length) {
								...
								$('#tablebox4').append("<div class='small table-responsive'><table id='tablbox4' class='table'><thead><tr style='background-color:#ff8000;color:white' class='p-0'><th  class='text-left'>Mesin</th><th class='text-md-right'>Harga</th></tr></thead></table></div>");
					...
					if(data[0]['akses']=='Y'){
						if ($('#tablbox4').length) {
							...
							for (i = 0; i < data.length; i++) {
								...
								x += "<tr class='text-md-left'><td>" + data[i]['mesin'] + "</td><td class='text-md-right' >"+drim+"<button type='submit' name='submit' value='" + [i] + "' class='btn btn-warning btn-sm' style='background-color:<?= $warnabar; ?>;color:white;width:120px'>Rp." + formatMoney(jual) + "</button><input type='hidden' name='jumlahcetak' value='" + jmlcetak + "' /><input type='hidden' name='ket' value='" + ket + "' /><input type='hidden' name='data1[]' value='" + arrStr + "' /><input type='hidden' name='data2[]' value='" + arrStr2 + "' /></td></tr>";
		...
		$("#cekukuran" + mod).click(function() {
			...
			if (jmlcetak == "" || jmlcetak == "0") {
				...
				salert('warning', 'Oops...', iMsg['W']+' harus diisi');
				} else if (idmesin == 0) {
				salert('warning', 'Oops...', iMsg['M']+' harus diisi');
				} else if (bahan == 0) {
				salert('warning', 'Oops...', iMsg['B']+' harus diisi');
				} else {
				// var jmlcetak = document.getElementById("jmlcetakbox4").value;
				var jmlwarna = document.getElementById("jmlwarnabox41").value;
