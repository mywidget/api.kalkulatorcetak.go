<?php
	...
	$lam = filterget('lam');
	// $bbstat = filterget('bb');
	$bbstat = (isset($_GET['bb']) ? filter($_GET['bb']) : 0);
	...
	// $s_email = "admin@kalkulatorcetak.com";
	if(!empty($s_email)){
		$database=mysql_query("SELECT profit,db_user,db_password,db_database FROM user WHERE email='".$s_email."'");
		$row_db=mysql_fetch_array($database);
		$persen = $row_db['profit'];	
		$serverHit = 'localhost';
		$usernameHit = $row_db['db_user'];
		$passwordHit = $row_db['db_password'];
		$databaseHit = $row_db['db_database'];
		
		mysql_connect($serverHit,$usernameHit,$passwordHit) or die("Koneksi gagal");
		mysql_select_db($databaseHit) or die("Database tidak bisa dibuka");
		}else{
	...
	function hitungcetak($lbrcetak,$tgcetak,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$finishing2, $finishing1, $finishing3,$finishing4,$finishing5,$finishing6,$finishing7,$finishing8,$finishing9,$finishing10,$submit,$lbrf1,$tgf1,$lbrf2,$tgf2,$lbrf3,$tgf3,$lbrf4,$tgf4,$lbrf5,$tgf5,$lbrf6,$tgf6,$lbrf7,$tgf7,$lbrf8,$tgf8,$lbrf9,$tgf9,$lbrf10,$tgf10,$modul,$jml_satuan,$cetak_bagi,$ket_satuan,$ongpot,$j_mesin,$kethitung,$jilid,$ongkos_lipat,$pakeplat,$persen){
		...
		if (empty($finishing2) or $finishing2==0) {
			...
			$sqlfinishing2 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing2'");
		...
		if (empty($finishing3) or $finishing3==0) {
			...
			$sqlfinishing3 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing3'");
		...
		if (empty($finishing4) or $finishing4==0) {
			...
			$sqlfinishing4 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing4'");
			$row5=mysql_fetch_array($sqlfinishing4);    
		...
		if (empty($finishing5) or $finishing5==0) {
			...
			$sqlfinishing5 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing5'");
			$row5=mysql_fetch_array($sqlfinishing5);    
		...
		if (empty($finishing6) or $finishing10==6) {
			...
			$sqlfinishing6 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing6'");
			$row6=mysql_fetch_array($sqlfinishing6);    
		...
		if (empty($finishing7) or $finishing7==0) {
			...
			$sqlfinishing7 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing7'");
			$row7=mysql_fetch_array($sqlfinishing7);    
		...
		if (empty($finishing8) or $finishing8==0) {
			...
			$sqlfinishing8 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing8'");
			$row8=mysql_fetch_array($sqlfinishing8);    
		...
		if (empty($finishing9) or $finishing9==0) {
			...
			$sqlfinishing9 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing9'");
			$row9=mysql_fetch_array($sqlfinishing9);    
		...
		if (empty($finishing10) or $finishing10==0) {
			...
			$sqlfinishing10 = mysql_query("select * from tbl_biaya where KdBiaya='$finishing10'");
			$row10=mysql_fetch_array($sqlfinishing10);    
		...
		while($row=mysql_fetch_array($sql)){ 
			...
			elseif($jset >=1 AND $sisa <= 0){
				...
				// if ($jilid < 1){ 
				// $hitc = hitbiayacetak($mesin, $lbrcetak,$tgcetak,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat );
				// }
				
				//		 echo "<br>" . json_encode($hita[0]) . "<br> <br>";
				//		 echo "<br>" . json_encode($hit_b[0]) . "<br> <br>";
				//		//$hit = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$jsett,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat );
			...
			elseif($jset>=1 AND $sisa > 0){
				...
				while($row2=mysql_fetch_array($sql2)){
					...
					//			 echo $sisa . " halaman muat " . $jml_pot_sisa . " (" . $uk_lbr_sisa . "x" . $uk_tg_sisa . "cm) di" . $mes2 . " - " .$jset_sisa ."(" . $jsett_sisa . ") set, sisa " . $sisa_akhir . "<br>";		
					...
					//$data2 = $gabung2[0];
					// echo ($ukuran_sisa[2]);	
					//	 echo "<br>hit2_a " . json_encode($hit2_a) . "<br> <br>";
					//	 echo "<br>hit2_b " . json_encode($hit2_b) . "<br> <br>";
				...
				//	 echo "<br> " . json_encode($data2[0]) . "<br> <br>";
		...
		//	echo json_encode($data10) . "<br><br><br>";	
		//	echo json_encode($datafinishing) . "<br><br><br>";	
		//array_push($data10,($datafinishing));
		//Total dulu nilai jml_total ama totfinishing
		...
		//echo json_encode($data6) . "<br><br><br>";	
		
		//array_push($data10,array($datafinishing));
	...
	function hitbiayacetak($mesin, $lbrcetak,$tgcetak,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbc,$tgc,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat ){
		...
		// Cari Jenis Bahan
		$sqljenis = mysql_query("select * from kategori_bahan where id_kategori='$bahan'");
		$rowjenis = mysql_fetch_array($sqljenis); // 
		...
		$sql = mysql_query("select * from mesin where kdmesin='$mesin' AND aktif='Y'");
		while($row=mysql_fetch_array($sql)){ //mesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesinmesin 
			...
			while ($ulangbb <= 3) {  //selama cetak bolak balik  xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
				...
				while ($z / $lebarpot >= 1){ // Selama Tinggi Mesin masih muat u/ Lebar Cetakan	// x aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa	
					...
					while ($z / $tinggipot >= 1) { //  Selama Tinggi Mesin masih muat u/ Tinggi Cetakan  // y
						...
						while($row2=mysql_fetch_array($sql2)){ //vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
							...
							if ($jmldrek < 0) 
							{  //jika jumlah drek lebih kecil dari 0
							$jmldrek = 0;
							}			
							...
							$sqlinsheet = mysql_query("select * from tbl_insheet where '$jmlbhn' between dari and sampai");			
							$rowinsheet=mysql_fetch_array($sqlinsheet);
							if($ins_bb == 2){
				...
				$y = 1;
				$ulangbb = $ulangbb + 1;
			} // endwhile cetak bb xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
