...
$('.modal').on('hidden.bs.modal', function() {
	$(this).removeData('bs.modal');
			...
			// $("#bahanam2").filter(function() {
				// var deptid = 10;
				// $.ajax({
					// url: host + "/api/"+app_id+"/ukuran/"+mod+"/0",
					// type: 'GET',
					// dataType: 'json',
					// success: function(response) {
						...
						// for (var i = 0; i < len; i++) {
							...
							// $("#bahanam2").append("<option value='" + id + "'>" + name + "</option>");
						// }
					// }
				// });
			// });
		...
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			// jika marketing jangan tampilkan ini
				if ($('#tablam2').length) {
					...
					$('#tableamp2').append("<div id='detailamp2' class='small'><table id='tablam2' class='table table-striped'><thead ><tr style='background-color:<?=$warnabar;?>;color:white' ><th  class='text-left' style='width:30%'>Mesin</th><th class='text-right'>Harga</th></tr></thead></table></div>");
					var table = $('#tablam2').children();
					var i;
					var no = 1;
					var ongkos_potong = 0;
