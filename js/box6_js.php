		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(bawah != 1 ){
					...
					if( $(ch_mod).length ){
					$(ch_mod+' tr:gt(0)').remove();	
					var table = $(ch_mod).children();					
					var i;
					var no=1;
					var x;
					var ongkos_potong = 0;
					for (i = 0; i < data.length; i++) {
						...
						x +="<tr id='ctr' class='text-md-left'><td>"+data[i]['mesin']+"</td><td id='ctd' class='text-md-right' >Rp. "+formatMoney(satuan)+"/pcs - Rp. "+formatMoney(perrim)+"/rim <button type='submit' name='submit' value='"+[i]+"' class='btn btn-warning btn-sm' style='width:120px'>Rp."+formatMoney(jual)+"</button></td><input type='hidden' name='jumlahcetak' value='"+jmlcetak+"' /><input type='hidden' name='ket' value='"+ket+"' /><input type='hidden' name='data1[]' value='"+arrStr+"' /></tr>";	
					...
					table.append(x);
						...
						$(app_mod).append("<div class='small table-responsive'><table id='"+id_mod+"' class='table table-striped' ><thead ><tr id='ctrh'><th id='cthh' class='text-md-left' style='width:30%'>Mesin</th><th class='text-md-right'>Harga</th></tr></thead></table></div>");
