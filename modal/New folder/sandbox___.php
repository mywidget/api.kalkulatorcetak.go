...
$json1 = json_decode($arraym);
// print_r($json1);
$pilihM1 = pilihM2($json1->mesin,$where);
foreach ($pilihM1 as $item) {
...
// foreach ($pilihM2 as $item) {
	...
	// print_r($ukuran_lipat);
	$uk_lbr = ($ukuran_lipat[0][0]);
	$uk_tg = ($ukuran_lipat[0][1]);
	$uk_lbr2 = ($ukuran_lipat[1][0]);//1
	$uk_tg2 = ($ukuran_lipat[1][1]);//2
	$jml_pot = ($ukuran_lipat[2]);	
	
	// $jml_pot = isset($ukuran_lipat[2]) ? $ukuran_lipat[2] : null;
	...
	if (is_null($set) or $set < 1){ 
	 ...
	 // echo "a";
		...
		$hit2 = hitbiayacetak($idmesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul);
	...
	elseif($jset >=1 AND $sisa <= 0){
		...
		$data=[];
		if (!empty($uk_lbr)){ 
			$hita = hitbiayacetak($idmesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul);
		} else { $hita = [];}
		if (!empty($uk_lbr2)){ 
			$hitb = hitbiayacetak($idmesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul);
		} else { $hitb = []; }
	...
	//	echo "<br>" . json_encode($ukuran_sisa) . "<br> <br>";
		...
		$data2=[];
			...
			if (!empty($uk_lbr_sisa)){
				$hit2_a = hitbiayacetak($idmesin, $uk_lbr_sisa,$uk_tg_sisa,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,($jsett_sisa),$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email ,$idkertas,$modul);
			} else { $hit2_a = []; }
			if (!empty($uk_lbr_sisa2)){
				$hit2_b = hitbiayacetak($idmesin, $uk_lbr_sisa2,$uk_tg_sisa2,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,($jsett_sisa),$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email ,$idkertas,$modul);
			} else { $hit2_b = []; }
...
function hitbiayacetak($mesin, $lbrcetak,$tgcetak,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbc,$tgc,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul){
...
$jsonm = json_decode(ArrayMesin($s_email));
$jsonm = $jsonm->mesin;
...
$item = pilihM2($jsonm,$mesin);

// foreach ($json->mesin as $item) {
		// // if($val_mesin['aktif']=='Y'){
		// if($item->kdmesin==$mesin AND $item->aktif=="Y"){
// echo $mesin;
		// echo $item->namamesin;
// echo "<br/>";
		$hargaminim = $item[0]['hargamin'];
		$jmlminim = $item[0]['jumlahmin'];
		$hargadrek = $item[0]['hargalebih'];
		$lbrkertasmesin = $item[0]['lebarkertas'];
		$tgkertasmesin = $item[0]['panjangkertas'];
		$lebartext = $item[0]['lebarcetak'];
		$tinggitext = $item[0]['panjangcetak'];
		$lebarmin = $item[0]['min_lebar'];
		$tinggimin = $item[0]['min_panjang'];
		$ctp = $item[0]['hargactp'];
		$NamaMesin = $item[0]['namamesin'];
		$jenis_mesin = $item[0]['jenis_mesin'];
		$replatsetiap = $item[0]['replat'];
		if (empty($replatsetiap)){
			$replatsetiap = 1;
		}
		if (is_null($tarik)){
			$tarik = $item[0]['tarikan'];
		}elseif ($tarik == 0){
			$tarik = 0;
		}else{
			$tarik = $item[0]['tarikan'];
	...
	if ($lebarmin > $tinggimin){
		$tinggimin = $item[0]['min_lebar'];
		$lebarmin = $item[0]['min_panjang'];}
	...
	if ($ulangbb <= 2){   //Jika di Cetak Bolak balik plat yang sama
		$cetakbb= 2;
		$ins_bb = 2;	
		$biayabbplatsama = $item[0]['biayabbplatsama'];		
		$jmlwarna = $jmlwarna1;
