...
foreach ($pilihM2 as $item) {
	...
	if (is_null($set) or $set < 1){ 
	 ...
	 // echo "a";
		...
		$hit2 = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul);
	...
	elseif($jset >=1 AND $sisa <= 0){
		...
		$data=[];
		if (!empty($uk_lbr)){ 
			$hita = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul);
		} else { $hita = [];}
		if (!empty($uk_lbr2)){ 
			$hitb = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email,$idkertas,$modul);
		} else { $hitb = []; }
	...
	//	echo "<br>" . json_encode($ukuran_sisa) . "<br> <br>";
		...
		$data2=[];
			...
			if (!empty($uk_lbr_sisa)){
				$hit2_a = hitbiayacetak($mesin2, $uk_lbr_sisa,$uk_tg_sisa,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,($jsett_sisa),$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email ,$idkertas,$modul);
			} else { $hit2_a = []; }
			if (!empty($uk_lbr_sisa2)){
				$hit2_b = hitbiayacetak($mesin2, $uk_lbr_sisa2,$uk_tg_sisa2,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,($jsett_sisa),$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email ,$idkertas,$modul);
			} else { $hit2_b = []; }
...
//					$muat = $x * $y * $cetakbb;
	//echo $x . " x " . $y . " = " . $lebarpot . " x " . $tinggipot . " - " . $NamaMesin . " Muat : " . $x * $y .  "<br>" ;
					...
					if($set == 1 AND $jmlmuat ==1){					
					$jmlset = 1;
					}else{
					$jmlmuat = $jmlmuat*2;	
					$jmlset = ceil($set/$jmlmuat);// * $cetakbb;
					// $jmlset = $set;// * $cetakbb;
					}
					// $muats = $muat *2;
					...
					if ($laminat==''){
						...
						$biayalam = $lbrcetak * $tgcetak * $jmlcetak * $hrglebihlam * $jmlam * $jmlset;						
...
// echo "J4: ".$jenis_mesin;
					if($jenis_mesin == 'PrintDigital' ){
							...
							$jmlprint = ceil(($jmlcetak * $jmlset * $jml_satuan)/$jmlpot);
...
// echo $row['harga'];
							...
							$totlaminating = $lbrcetak * $tgcetak * $jmlcetak * $jml_satuan * $hrglebihlam * $jmlam * $jmlset;
							...
							$jmlplano = ceil((($jmlbhn + $insheet_) * $jmlset )/ $jmlpot2);
					$totbhn = 0;
							
					}else{
							$totcetak = ceil((($hargaminim * $jmlwarna) + $totdrek + ($biayabbplatsama * $jmlwarna)) * $jmlset );
							$jmlplano = ceil((($jmlbhn + $insheet_) * $jmlset ) / $jmlpot);
...
// echo "<br/>";
					...
					$lipat = ($hrgfinishing10lebih + (($hrgfinishing10lebih/2) * (($jmlmuat/2) - 1))) *    $jmlbhn * $jmlset;
