...
$pakeplat = !empty($type->pakeplat)?$type->pakeplat:0;

// $idmesin = $type->idmesin;
// $idkertas = $type->idkertas;
//
$tarik = !empty($type->tarikan)?$type->tarikan:0;
...
$jsonm = $jsonm['mesin'];
$item = pilihMmesinMod($jsonm,$modul);
...
foreach ($pilihM2 as $items) {
	$jenis_mesin = $items['jmesin'];
...
// $row1=pilihBiaya($ArrayBiaya['biaya'],12);    
// print_r($row1);
...
// print_r($item);
// foreach ($json->mesin as $row) {
	
 // $json = json_decode($arraym);
// $pilihM2 = pilihM2($json->mesin,$where);
...
// print_r($row);
	...
	if (is_null($set) or $set < 1){ 
	 ...
	 // echo "a";
		...
		$hit2 = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email);
	...
	elseif($jset >=1 AND $sisa <= 0){
		...
		$data=[];
		if (!empty($uk_lbr)){ 
			$hita = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email);
		} else { $hita = [];}
		if (!empty($uk_lbr2)){ 
			$hitb = hitbiayacetak($mesin, $uk_lbr,$uk_tg,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email);
		} else { $hitb = []; }
...
//		echo "<br>" . json_encode($data) . "<br> <br>";
	...
	//	echo "<br>" . json_encode($ukuran_sisa) . "<br> <br>";
		...
		$data2=[];
			...
			// $mes2 = $item2['namamesin'];
			if (!empty($uk_lbr_sisa)){
				$hit2_a = hitbiayacetak($mesin2, $uk_lbr_sisa,$uk_tg_sisa,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,($jsett_sisa),$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email );
			} else { $hit2_a = []; }
			if (!empty($uk_lbr_sisa2)){
				$hit2_b = hitbiayacetak($mesin2, $uk_lbr_sisa2,$uk_tg_sisa2,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,($jsett_sisa),$lam,$lbrcetak,$tgcetak,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email );
			} else { $hit2_b = []; }
...
//	echo json_encode($datafinishing) . "<br><br><br>";	
	...
	foreach($data10 as $key => $value){
	// echo $value['jml_total'];
				// $tot = intval($value['jml_total']) + intval($datafinishing['finishing']['totfinishing']);	
				$tot = intval($data10[$key]['jml_total']) + intval($datafinishing['finishing']['totfinishing']);	
				$totjual = ($tot * $persen/100) + $tot;
				$data10[$key]['jml_total'] = $tot;
				$data10[$key]['total_jual'] = $totjual;	
				// echo $totjual;
	...
	foreach($data10 as $key => $value){
			//for($i = 0 ; $i<count($key); $i++){
					$data2 = array_merge($data10[$key],$datafinishing['finishing']);
					array_push($data6,$data2);
			//}
		} //End foreach	
	$data10 = $data6;
...
function hitbiayacetak($mesin, $lbrcetak,$tgcetak,$jmlcetak,$bahan,$jmlwarna,$jmlwarna2,$bbstat,$tarik,$insheet,$set,$lam,$lbc,$tgc,$jml_satuan,$cetak_bagi,$ket_satuan,$ongkos_lipat,$pakeplat,$s_email){
...
$json = json_decode($ArrayMesinz);
print_r($json);
// foreach ($json->mesin as $item) {
$pilihM2 = pilihM2($json->mesin,$mesin);
foreach ($pilihM2 as $row) {
