    ...
    <div class="col-md-12">
        ...
        <div class="form-group">
          <div class="input-group">
            <span class="input-group-addon">Jumlah Cetak
            </span>
        ...
        <div class="form-group ukuran">
          <div class="input-group">
            <span class="input-group-addon">Ukuran cetak</span>
...
$("#ukuran").filter(function() {
       var deptid = 10;
        $.ajax({
            url: 'http://kalkulator.go/user/api/ukuran/brosur/10',
            type: 'GET',
            dataType: 'json',
 ...
 <select name="idmesin" id="idmesin" class="custom-select form-control" data-mesin="http://kalkulator.go/user/api/mesin/brosur/0" data-valueKey="id" data-displayKey="name" required>	 
</select>
          </div>
        </div>
 ...
 <select name="bahan" id="bahan" class="custom-select form-control" data-source="http://kalkulator.go/user/api/katbahan/brosur/0" data-valueKey="id" data-displayKey="name" required>	 
</select>
          </div>
        </div>
        </div>
		<div class="col-md-6">
<div class="input-group">
	<span class="input-group-addon">Uk.plano</span>
				...
				myArr = JSON.parse(xmlhttp.responseText);
					...
					data = myArr;
								...
								var ongkos_potong = 0;
									...
									if (data.ongpot  == 'Y' && data.beratkertas !=0){
										ongkos_potong = data.ongkos_potong;
									}			
									totalk = parseInt(data.totcetak) + parseInt(data.totbhn) + parseInt(ongkos_potong) + parseInt(data.tot_ctp) + parseInt(data.totlaminating) + parseInt(data.finishing1) + parseInt(data.finishing2) + parseInt(data.finishing3) + parseInt(data.finishing4) + parseInt(data.finishing5) + parseInt(data.finishing6) + parseInt(data.finishing7) + parseInt(data.finishing8) + parseInt(data.finishing9);
									profits = data.persen;
									
									jual = (parseInt(totalk) * profits / 100) + parseInt(totalk);
									satuan = jual / jmlcetak / jml_satuan;
									perrim = satuan *  rim;
									
									var arrStr = btoa(encodeURIComponent(JSON.stringify(data)));
									
									x +="<tr class='text-md-left'><td>"+data.mesin+"</td><td class='text-md-right' >Rp. "+formatMoney(satuan)+"/pcs - Rp. "+formatMoney(perrim)+"/rim <button type='submit' name='submit' value='0' class='btn btn-warning btn-sm' style='background-color:<?=$warnabar;?>;color:white;width:120px'>Rp."+formatMoney(jual)+"</button></td><input type='hidden' name='jumlahcetak' value='"+jmlcetak+"' /><input type='hidden' name='ket' value='"+ket+"' /><input type='hidden' name='data1[]' value='"+arrStr+"' /></tr>";
								...
								var ongkos_potong = 0;
									...
									var arrStr = btoa(encodeURIComponent(JSON.stringify(data)));
									x +="<tr class='text-md-left'><td>"+data.mesin+"</td><td class='text-md-right' >Rp. "+formatMoney(satuan)+"/pcs - Rp. "+formatMoney(perrim)+"/rim <button type='submit' name='submit' value='' class='btn btn-warning btn-sm' style='background-color:<?=$warnabar;?>;color:white;width:120px'>Rp."+formatMoney(jual)+"</button></td><input type='hidden' name='jumlahcetak' value='"+jmlcetak+"' /><input type='hidden' name='ket' value='"+ket+"' /><input type='hidden' name='data1[]' value='"+arrStr+"' /></tr>";
								table.append(x);
								}
			}
			}
