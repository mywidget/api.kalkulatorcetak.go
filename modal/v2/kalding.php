...
<div class="modal-header" style="background-color:<?=$warnabar;?>;color:white;">
	<button type="button" class="close" id="kk" data-dismiss="modal" aria-hidden="true">&times;</button>
	<h4 class="modal-title">Cetak Kalender Dinding</h4>
</div>
...
<div class="col-md-12">
									...
									<div class="col-md-12">
										...
										<div class="input-group">
											...
											<?php $sql_ukur = mysql_query("SELECT * FROM ukuran_kertas where modul like '%kalding%' ORDER BY ket_ukuran"); ?>
										...
										<div class="input-group">
											...
											<?php $sql_bhn = mysql_query("SELECT * FROM kategori_bahan where modul like '%kalding%' AND pub='Y' ORDER BY id_kategori"); ?>
										...
										<div class="col-md-6">
										<div class="form-group">
											<?php $sql_kal = mysql_query("SELECT * FROM tbl_biaya WHERE tbl_biaya.Groups LIKE '%kalender%' ORDER BY Nama_Biaya"); ?>
											<select id="finishing1k"  class="chosen-select form-control" required="required">
												<?php while($rowf=mysql_fetch_array($sql_kal)){ ?>
...
//jika spiral
	...
	//Hitung Isinya----------------------------------------	
		...
		if(percentComplete){setTimeout(function(){$(".progress-bar").css('background','green');$('#tablekalding').show();}, 500);}}},false);
